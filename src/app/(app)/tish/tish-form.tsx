...
export function TishForm() {
  ...
  async function onSubmit(data: ServiceSessionFormValues) {
    ...
    try {
        const { activityData, uploadTasks } = await prepareActivityData(data, 'TISH');
        await addActivity(firestore, user.uid, currentUserProfile.district, activityData, uploadTasks);

        toast({
        title: "Service Session Saved!",
        description: "The new TISH service session has been recorded.",
        });
        form.reset(defaultValues);
        setDuration(null);
        setFileUploadKey(Date.now());
    } catch (error: any) {
        console.error("Failed to save TISH session:", error);
        toast({
          title: "Save failed",
