...
const getTopicDisplayNames = (talk: any) => {
    const { topics, otherTopic } = talk.details;
    if (!topics || topics.length === 0) return "No topics";
    
    return topics.map((t: any) => {
        if (t.id === 'other') {
            return `${otherTopic || 'Other'} (${t.peopleReached || 0})`;
        }
...
export function HealthTalksHistory() {
  ...
  const talks = React.useMemo(() => {
    ...
    return activities
      .filter(a => {
          if (a.type !== 'Health Talk') return false;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>This Month's Health Talks</CardTitle>
        <CardDescription>A list of health talks recorded this month.</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="flex flex-col h-60 items-center justify-center rounded-md border border-dashed text-center">
            ...
            <p className="text-muted-foreground">
              No health talks recorded this month.
            </p>
          </div>
        )}
      </CardContent>
