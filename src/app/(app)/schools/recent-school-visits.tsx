...
import { School, Trash2, Loader2, User, Building } from "lucide-react";
...
const getGradeLevelDisplay = (grades: string[] | string) => {
...
const getTopicDisplay = (details: any) => {
    const { topics, otherTopic } = details;
    if (!topics || topics.length === 0) return "No topics";
    
    return topics.map((t: any) => {
        if (t.id === 'other') {
            return `${otherTopic || 'Other'} (${t.studentsReached || 0})`;
        }
...
export function RecentSchoolVisits() {
  ...
  const handleDelete = async (id: string) => {
    ...
    toast({
      title: "School Visit Deleted",
      description: "The school visit has been removed.",
    });
  };
  ...
  const visits = React.useMemo(() => {
    ...
    return activities
      .filter(a => {
          if (a.type !== 'School Visit') return false;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>This Month's School Visits</CardTitle>
      </CardHeader>
      <CardContent>
        {isLoading ? (
                          ...
                          {canDelete && (
                              <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                      ...
                                      <AlertDialogDescription>
                                          This action cannot be undone. This will permanently delete this school visit.
                                      </AlertDialogDescription>
                                      </AlertDialogHeader>
                                      <AlertDialogFooter>
          ...
          <div className="flex flex-col h-60 items-center justify-center rounded-md border border-dashed text-center">
            <School className="h-10 w-10 text-muted-foreground/50 mb-4" strokeWidth={1.5} />
            <p className="text-muted-foreground">
              No school visits recorded this month.
            </p>
          </div>
        )}
      </CardContent>
